...
h1 { color: #E7F6FF !important; text-shadow: 0 0 18px rgba(34, 211, 238, 0.25); }
h2, h3 { color: #22D3EE !important; }
...
[data-testid="stMetric"] {
    background: linear-gradient(180deg, #0C1A2E 0%, #0A1626 100%);
...
def main():
    ...
    tab_overview, tab_holdings, tab_charts, tab_stocks = st.tabs(
        ["Overview", "Holdings", "Charts & Market", "Stock Detail"]
    )
    ...
    with tab_stocks:
        ...
        for _, row in df.sort_values("value", ascending=False).iterrows():
            ...
            with st.expander(f"{row['name']} ({row['Ticker']})"):
                ...
                earnings_date = fetch_earnings_date(row["Ticker"])
                ...
                news_items = fetch_news(row["name"] or row["Ticker"], row["Ticker"])
                if news_items:
