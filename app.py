...
html, body, [class*="css"] {
    ...
    background-color: #C7DFF5 !important;
    font-weight: 500;
}
p, span, div, label, li {
    font-weight: 500 !important;
}
.stCaption, [data-testid="stCaptionContainer"], [data-testid="stMarkdownContainer"] p {
    font-weight: 500 !important;
}
...
h2, h3 { color: #1D6FD1 !important; font-weight: 700 !important; }
...
[data-testid="stMetric"] {
    background: linear-gradient(180deg, #AFCFEE 0%, #9FC3E8 100%);
    border: 1px solid rgba(29, 111, 209, 0.25);
    border-radius: 10px;
    padding: 14px 16px;
}
...
div[data-testid="stVerticalBlockBorderWrapper"] {
    ...
    border: 1px solid rgba(29, 111, 209, 0.25) !important;
    border-radius: 10px;
}
...
div[data-baseweb="tab-highlight"] {
    background-color: #1D6FD1 !important;
}
div[data-baseweb="tab-border"] {
    background-color: #8FB9E3 !important;
}

hr { border-color: #8FB9E3 !important; }
...
def load_watchlist() -> pd.DataFrame:
    ...
    return pd.DataFrame([{"Ticker": "NVDA", "Price When Added": None}])
...
def theme_chart(fig):
    fig.update_layout(
        paper_bgcolor="#C7DFF5",
        plot_bgcolor="#C7DFF5",
        font=dict(family="Inter, sans-serif", color="#10243D"),
        legend=dict(font=dict(color="#10243D")),
    )
    return fig
...
def render_stock_card(
    ticker: str, name: str, info: dict, currency_symbol: str = "£",
    reference_price: float = None, reference_label: str = "Your buy price",
    context: str = "default",
):
    ...
    If reference_price is given (e.g. buy price or watch price), draws it as
    a dashed line on the chart for comparison."""
    sym = currency_symbol
    ...
    period_choice = price_cols[2].radio(
        ...
        horizontal=True, key=f"period_{context}_{ticker}", label_visibility="collapsed",
    )
    ...
    if not hist.empty:
        ...
        if reference_price is not None:
            fig.add_hline(
                ...
                line_dash="dot",
                line_color="#FBBF24",
                annotation_text=f"{reference_label}: {sym}{reference_price:,.2f}",
                annotation_position="top left",
                annotation_font_color="#FBBF24",
            )
        fig.update_layout(
            height=220, margin=dict(l=0, r=0, t=10, b=0),
            xaxis=dict(gridcolor="#8FB9E3", showgrid=False),
            yaxis=dict(gridcolor="#8FB9E3"),
            showlegend=False,
        )
        st.plotly_chart(theme_chart(fig), use_container_width=True, key=f"chart_{context}_{ticker}_{period_choice}")
    else:
...
def main():
    ...
    with tab_charts:
        ...
        if not bench.empty:
            ...
            fig3.update_layout(
                ...
                xaxis=dict(gridcolor="#8FB9E3"), yaxis=dict(gridcolor="#8FB9E3"),
            )
    ...
    with tab_stocks:
        ...
        for _, row in df.sort_values("value", ascending=False).iterrows():
            ...
            with st.expander(f"{row['name']} ({row['Ticker']})"):
                render_stock_card(
                    row["Ticker"], row["name"], info, row.get("currency_symbol", "£"),
                    reference_price=row.get("Avg Price"), reference_label="Your buy price",
                    context="holdings",
                )
    ...
    with tab_watchlist:
        ...
        edited_watch = st.data_editor(
            ...
            column_config={
                "Ticker": st.column_config.TextColumn(required=True),
                "Price When Added": st.column_config.NumberColumn(
                    format="%.2f", help="What the price was when you started watching it — shown as a line on the chart"
                ),
            },
        ...
        watch_rows = edited_watch.dropna(subset=["Ticker"]).copy()
        watch_rows = watch_rows[watch_rows["Ticker"].str.strip() != ""]
        watch_rows["Ticker"] = watch_rows["Ticker"].str.strip().str.upper()
        watch_rows = watch_rows.drop_duplicates(subset=["Ticker"], keep="first")
        watch_tickers = watch_rows["Ticker"].tolist()
        ...
        if not watch_tickers:
            ...
            st.divider()
            for _, wrow in watch_rows.iterrows():
                t = wrow["Ticker"]
                ...
                price_when_added = wrow.get("Price When Added")
                if pd.isna(price_when_added):
                    price_when_added = None
                with st.expander(f"{info.get('name', t)} ({t}) — {sym}{info['current_price']:,.2f}{change_str}"):
                    render_stock_card(
                        t, info.get("name", t), info, sym,
                        reference_price=price_when_added, reference_label="Price when added",
                        context="watchlist",
                    )
            ...
            if region_df.empty:
                ...
                for _, r in region_df.iterrows():
                    ...
                    with st.expander(f"{r['name']} ({r['ticker']}) — analysts see {r['upside_pct']:+.1f}% upside"):
                        render_stock_card(r["ticker"], r["name"], info, sym, context=f"discover_{region}")
